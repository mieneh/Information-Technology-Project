...
import { FaHome, FaTags, FaArchive, FaTasks, FaMapMarkerAlt, FaBoxes, FaUserCircle, FaAddressBook, FaPhone, FaEnvelope } from 'react-icons/fa';
...
const Produce = () => {
    ...
    const fetchProducers = async () => {
        try {
          const response = await axios.get('http://localhost:3000/api/users/producer');
    ...
    const fetchConnections = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:3000/api/connection/producer', {
    ...
    const handleShowPhone = (producer = null) => {
        setShowPhone(true);
        window.location.href = `tel:${producer.contactPhone}`;
    };

    const handleShowRequest = (producer) => {
    ...
    const handleSendRequest = async () => {
        const existingRequest = connections.find((connection) =>
            connection.producerID?._id === selectedProduce._id && connection.status === 'Pending'
        );
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('http://localhost:3000/api/connection/send-producer', {
                producerID: selectedProduce._id,
                message: message,
            }, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleAcceptRequest = async (id) => {
        try {
            ...
            setConnections(connections.map(collab => 
                collab._id === id ? { ...collab, status: 'Accepted' } : collab
            ));
        } catch (error) {
    ...
    const handleRejectRequest = async (id) => {
        try {
            ...
            setConnections(connections.filter(collab => collab._id !== id));
        } catch (error) {
    ...
    const handleCancelRequest = async (id) => {
        try {
            ...
            setConnections(prevState => prevState.filter(collab => collab._id !== id));
        } catch (error) {
            ...
            {role === "Producer" ? (
                ...
                <div style={{ padding: '20px' }}>
                    <div className="header-container">
                        <h2 className="header-title">Đơn Vị Sản Xuất</h2>
                    ...
                    {producers.length === 0 ? (
                        ...
                        .map((producer) => (
                            <div key={producer._id} className="card">
                                <SwipeToRevealActions
                                    actionButtons={[
                                        {
                                            content: <FaEnvelope style={{ fontSize: '30px', color: 'green' }} />,
                                            onClick: () => handleShowRequest(producer),
                                        },
                                        {
                                            content: <FaPhone style={{ fontSize: '30px', color: 'red' }} />,
                                            onClick: () => handleShowPhone(producer),
                                        },
                                    ]}
                                    actionButtonMinWidth={60}
                                    style={{ backgroundColor: 'white' }}
                                >
                                    <div className="info" onClick={() =>setSelectedProduce(producer === selectedProduce ? null : producer)}>
                                        <h3>{producer.farmName}</h3>
                                        <p>{producer.farmLocation}</p>
                                    </div>
                                </SwipeToRevealActions>
                                {selectedProduce && selectedProduce._id === producer._id && (
                                        ...
                                        {selectedProduce && (
                                            <Form>
                                                <Form.Group className="mb-3">
                                                    ...
                                                    <Form.Control
                                                        value={selectedProduce.farmName}
                                                        readOnly
                                                    />
                                                </Form.Group>
                                                <Form.Group className="mb-3">
                                                ...
                                                <Col sm={12}>
                                                    <Tab.Content style={{boxShadow: 'none', padding: '0px'}}>
                                                        <Tab.Pane eventKey="Sent">
                                                            {connections.filter(c => c.status === 'Pending' && c.typeTransporter === 'Sent').length === 0 ? (
                                                                <p>Chưa có yêu cầu đã gửi.</p>
                                                            ) : (
                                                                connections
                                                                .filter(c => c.status === 'Pending' && c.typeTransporter === 'Sent')
                                                                .map((connection) => (
                                                                    <div key={connection._id}>
                                                                        <div className="connection-card">
                                                                            <div className="connection-header d-flex align-items-center justify-content-between">
                                                                                <div className="avatar d-flex align-items-center">
                                                                                    {connection.producerID?.image ? (
                                                                                        <img src={connection.producerID.image} alt={connection.producerID.farmName} className="avatar-image" />
                                                                                    ) : (
                                                                                        <FaUserCircle size={40} style={{ color: '#4caf50' }} />
                                                                                    )}
                                                                                </div>
                                                                                <div className="connection-info">
                                                                                    <h5 className="connection-name">{connection.producerID?.farmName}</h5>
                                                                                    <p className="connection-subinfo">{connection.producerID?.farmLocation || "Địa chỉ không xác định"}</p>
                                                                                </div>
                                                        ...
                                                        <Tab.Pane eventKey="Received">
                                                            {connections.filter(c => c.status === 'Pending' && c.typeTransporter === 'Received').length === 0 ? (
                                                                <p>Chưa có yêu cầu đã nhận.</p>
                                                            ) : (
                                                                connections
                                                                .filter(c => c.status === 'Pending' && c.typeTransporter === 'Received')
                                                                .map((connection) => (
                                                                    <div key={connection._id}>
                                                                        <div className="connection-card">
                                                                            <div className="connection-header d-flex align-items-center justify-content-between">
                                                                    ...
                                                                    <div key={connection._id}>
                                                                        <div className="connection-card">
                                                                            <div className="connection-header d-flex align-items-center justify-content-between">
                                                                                ...
                                                                                <div className="connection-buttons">
                                                                                    <button className="btn-danger" onClick={() => handleRejectRequest(connection._id)}>Xóa</button>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
